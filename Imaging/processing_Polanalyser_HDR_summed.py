...
imgs_raw  = [tiff.imread(os.path.dirname(imfile)+'/'+imfl) for imfl in tiffs[0]]#0 means greyscale
...
img_mid_over = np.where(img_mid > np.round(255*int(sys.argv[1])*(1-edge_lim/100))-1)
img_mid_under = np.where(img_mid < np.round(255*int(sys.argv[1])*(edge_lim/100))-1)
...
"""
## THIS IS FOR DARK IMAGES
img_HDR_val = img_HDR_msk.ravel()
img_HDR_val = img_HDR_val[img_HDR_val > 0]

log_HDR_val = np.log10(img_HDR_val)

bins = 1000  
HDR_histogram = np.histogram(log_HDR_val, bins=bins)

fig, ax = plt.subplots()
ax.bar(HDR_histogram[1][:-1], HDR_histogram[0], 
       width=np.diff(HDR_histogram[1]), 
       color='tab:blue', align='edge', edgecolor='k')

ax.set_title('HDR Image Histogram (Log10 Transformed)')
ax.set_xlabel("Log10 Pixel Intensity")
ax.set_ylabel("Frequency")

output_dir = os.path.dirname(imfile)
fig.savefig(os.path.join(output_dir, 'HDR_histogram_log10.pdf'))
np.savetxt(os.path.join(output_dir, "HDR_histogram_log10.csv"), 
           np.column_stack((HDR_histogram[1][:-1], HDR_histogram[0])),
           delimiter=',', header="Log10 Bin Edges, Frequency", comments='')

## THIS FOR BRIGHER IMAGES
img_HDR_val = img_HDR_msk.ravel()
fig = plt.figure()
