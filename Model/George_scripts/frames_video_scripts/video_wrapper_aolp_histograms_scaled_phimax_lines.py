...
# this script makes frames for each rotation angle (of the original image) as well as polar histograms. In each ommatidium there are lines that correspond to the phi_max values. These lines are the same for each rotation, so
...
for rotation_angle in range(0, 360, 5):
    ...
    with open("azimuth_elevation_slope_data.csv", 'r') as phimax_values:
        for line in phimax_values:
            ...
            phimax_2ndeye.append(float(float(x[3])*(-1)))
