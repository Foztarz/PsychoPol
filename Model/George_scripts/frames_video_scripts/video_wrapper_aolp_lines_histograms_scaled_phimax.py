...
for rotation_angle in range(0, 360, 5):
    ...
    with open("azimuth_elevation_slope_data.csv", 'r') as phimax_values:
        for line in phimax_values:
            x = line.split('\t')
            phimax_list.append(float(x[3])) # to not have the +e
        print(phimax_list)
    ...
    os.system('python realistic_FOV_aep_tissot_multiple_colors_2ndeye_phimax_lines.py ' +args.input + ' aolp_2ndeye_' + str(rotation_angle) + '_lines.png "' + str(azimuth_deg_list) + '" "' + str(elevation_deg_list) + '" "' + str(phimax_list) + '"')
