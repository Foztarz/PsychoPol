...
def main():
    ...
    # Apply atan2(sin(a), abs(cos(a))) transform
    a_deg = np.abs(grouped['error'].values)  # absolute error in degrees
    a_rad = np.radians(a_deg)                # convert to radians
    s = np.sin(a_rad)
    c = np.cos(a_rad)
    errors = np.degrees(np.arctan2(s, np.abs(c)))  # final transformed error in degrees (bimodal error)
    ...
    cmap_plot = ax.pcolormesh(theta_grid, r_grid, error_grid, cmap="plasma", shading='auto', vmin=0, vmax=90)
    ...
    plt.colorbar(cmap_plot, ax=ax, label='Sun azimuth estimate - Bimodal Error (deg)')
