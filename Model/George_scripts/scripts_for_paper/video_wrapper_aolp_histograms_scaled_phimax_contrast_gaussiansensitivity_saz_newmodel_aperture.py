...
def forstokes_eye_1(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_gaussian_sensitivity_aperture.py ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads + ' ' + args.sigma), shell=True, stdout=subprocess.PIPE, text=True) as process:
...
def forstokes_eye_2(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_2ndeye_gaussian_sensitivity_aperture.py ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads + ' ' + args.sigma), shell=True, stdout=subprocess.PIPE, text=True) as process:
        for line in process.stdout:
            if '000' in img_dmc:
...
for rotation_angle in range(55, 360, 5):
    ...
    # first eye
    dolp = [math.sqrt(x2**2 + y2**2) / z2 if z2 != 0 else 0 for x2, y2, z2 in zip(S1, S2, S0)] # first check if S0 equals to 0
    dolp_2 = [math.sqrt(x2**2 + y2**2) / z2 if z2 != 0 else 0 for x2, y2, z2 in zip(S1_2, S2_2, S0_2)]

    if any(num > 0 for num in dolp) or any(num2 > 0 for num2 in dolp_2):
        PRC = [] # photoreceptor contrast first eye
        ...
        command = str('python realistic_FOV_aep_tissot_multiple_colors_aolp_phimax_contrast_saz_newmodel_aperture.py ' + args.input + ' aolp_1steye_' + str(rotation_angle) + '.png "' + str(azimuth_deg_list) + '" "' + str(elevation_deg_list) + '" "' + str(PRC) + '" 25 ' + str(rotation_angle) + ' "' + str(PRC_scaled) + '"')
...
print(len(np.repeat(rotation_angles, len(PRC))))
print(len(flat_PRC_values))
