...
def forstokes_eye_1(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_gaussian_sensitivity_integration.py ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads), shell=True, stdout=subprocess.PIPE, text=True) as process:
...
def forstokes_eye_2(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_2ndeye_gaussian_sensitivity_integration.py ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads), shell=True, stdout=subprocess.PIPE, text=True) as process:
        for line in process.stdout:
            if '000' in img_dmc:
