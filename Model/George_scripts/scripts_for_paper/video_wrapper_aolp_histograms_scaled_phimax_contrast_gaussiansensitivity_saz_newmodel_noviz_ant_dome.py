...
with open("azimuth_elevation_slope_data_ant_dome.csv", 'r') as crd: ## this is for using the pitched (zenith-looking) coordinates to the vector direction calculation. we shouldn't use the original azimuths and elevations
    for line in crd:
        x=line.split('\t')
...
for rotation_angle in range(0, 360, 5):
    ...
    if len(phimax_list) == 0: # don't run the script if it already exists
        with open("azimuth_elevation_slope_data_ant_dome.csv", 'r') as phimax_values:
