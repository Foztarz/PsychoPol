...
azimuth_deg_list = []
elevation_deg_list = []
azimuth_deg_list_pitched = []
elevation_deg_list_pitched = []
...
with open("azimuth_elevation_slope_data_bombus.csv", 'r') as crd: ## this is for using the pitched (zenith-looking) coordinates to the vector direction calculation. we shouldn't use the original azimuths and elevations
    for line in crd:
        x=line.split('\t')
        azimuth_deg_list_pitched.append(float(x[0]))
        elevation_deg_list_pitched.append(float(x[1].strip()))
...
for rotation_angle in range(0, 360, 5):
    ...
    if len(phimax_list) == 0: # don't run the script if it already exists
        with open("azimuth_elevation_slope_data_bombus.csv", 'r') as phimax_values:
