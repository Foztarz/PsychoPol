...
f4 = open("minimumfanshapemismatch_botheyes.tsv", "a", newline="") # for storing data regarding fan-shape-AoLP mismatch
f5 = open("mean_PRC_botheyes.tsv", "a", newline="") # for storing data regarding mean PRC (signed) of both eyes

writer1 = csv.writer(f1, delimiter="\t")
writer2 = csv.writer(f2, delimiter="\t")
writer3 = csv.writer(f3, delimiter="\t")
writer4 = csv.writer(f4, delimiter="\t")
writer5 = csv.writer(f5, delimiter="\t")
...
def forstokes_eye_1(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_honeybee_sensitivity.py ' + args.splinedata + ' ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads), shell=True, stdout=subprocess.PIPE, text=True) as process:
...
def forstokes_eye_2(img_dmc):
    ...
    with subprocess.Popen(str('python realistic_FOV_aep_tissot_multiple_omm_intensities_forstokes_2ndeye_honeybee_sensitivity.py ' + args.splinedata + ' ' + img_dmc + ' ' + args.coordinates + ' 25 ' + str(rotation_angle) + ' ' + args.threads), shell=True, stdout=subprocess.PIPE, text=True) as process:
        for line in process.stdout:
            if '000' in img_dmc:
...
for rotation_angle in range(0, 360, 5):
    ...
    for aolp_value, dolp_value, phimax_value, phimax_2_value in zip(aolp_lines, dolp, phimax_list, phimax_2_list):
        S_list.append(float(1 + ((dolp_value * (Sp - 1)) / (Sp + 1)) * 
                             np.cos(2 * (np.radians(270) - aolp_value) - 2 * np.radians(phimax_value))))
        S2_list.append(float(1 + ((dolp_value * (Sp - 1)) / (Sp + 1)) * 
                              np.cos(2 * (np.radians(270) - aolp_value) - 2 * np.radians(phimax_2_value))))
    ...
    mean_PRC_2 = statistics.mean(PRC_2)
    ...
    mean_PRC_botheyes = (mean_PRC + mean_PRC_2)/2

    writer5.writerow([mean_PRC_botheyes, absolute_error, float(args.solarazimuth)+float(rotation_angle)])
