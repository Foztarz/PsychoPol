...
##
##    s = Sky(theta_s=np.pi/3, phi_s=np.pi/4)
##    # s = sky_from_observer()
##    # s = Sky.from_type(11)
##    # s.theta_s = np.pi/6
##    y, p, a = s(t, p)
...
##    S0, S1, S2 = compute_stokes(s)
##    #visualise_stokes(s)
##    #visualise_all_polarizers(s)
##    save_fisheye_sky(s, resolution=1636)

if __name__ == "__main__":
    s = Sky(theta_s = np.radians(90-float(sys.argv[1])), phi_s=-np.radians(float(sys.argv[2])))
    resolution = 1683
