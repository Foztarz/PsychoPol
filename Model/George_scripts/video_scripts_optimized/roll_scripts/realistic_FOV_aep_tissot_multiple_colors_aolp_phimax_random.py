...
def main(image_path, output_path, azimuth_list, elevation_list, color_value_list, minor_axis):
    try:
        ...
        data_array[:, 4] = np.random.uniform(0, 180, len(results))
        data_array[:, 3] = data_array[:, 4] + 90

        if not os.path.exists("azimuth_elevation_phimax_random_data.csv"): # this is to not create a random phi max arrangement every time
            np.savetxt("azimuth_elevation_phimax_random_data.csv", data_array, delimiter="\t", comments="")
