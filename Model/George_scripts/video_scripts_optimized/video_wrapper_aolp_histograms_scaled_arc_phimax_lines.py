...
for rotation_angle in range(0, 360, 5):
    ...
    with open("azimuth_elevation_arc_intercepts_data.csv", 'r') as phimax_values:
        for line in phimax_values:
            ...
            phimax_2ndeye.append(float(float(x[4])*(-1))) # this (-1) is to  create the mirrored fan shaped arrangement in the second eye
    os.system('python realistic_FOV_aep_tissot_multiple_colors_phimax_lines.py ' + args.input + ' aolp_1steye_' + str(rotation_angle) + '_lines.png "' + str(azimuth_deg_list) + '" "' + str(elevation_deg_list) + '" "' + str(phimax_2_list) + '"')
