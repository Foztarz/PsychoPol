...
#
#  DESCRIPTION: Adapted from "Irradiance Apol03-20210331.R"
...
ignore_st <- 'dark'
#substitute this string from filenames
subst_st <- 'pc'
#with this string
repl_st <- '%'
...
spln.lmx <- StavengaSpline(c(300, 700), lmx) # an a1 opsin with a 540nm max absorption
...
PDFsave(Directory = spc, PlotName = paste('Stavenga template lambda max', lmx))
...
title(main = paste(mnm, 'as measured at ', mds,' mm'))
legend(lmx+50, up.photon, rev(snm), col = rev(clz[1:length(snm)]), lty = 1, lwd = 2)
...
legend(lmx+50, log10(up.photon), rev(snm), col = rev(clz[1:length(snm)]), lty = 1, lwd = 2)
legend(lmx+50, 9.5, 'Sunlit sky \nfrom Johnsen et al. 2008', col = 'skyblue', lty = 1, lwd = 5, bty = 'n')
