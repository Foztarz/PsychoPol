...
{path_folder = "C:\\Users\\jaf54iq\\Documents\\AllTestdata\\TestFolder\\dat_folder"}
...
pre_pdfs = list.files(path = path_folder,
                      ...
                      recursive = TRUE)
invisible({#delete each one without echoing to command window
lapply(X = file.path(path_folder,
                     pre_pdfs
                     ),
       FUN = file.remove
       )
...
rast_plot = FT_raster_condition(
                        ...
                        show_plot = FALSE, #Tell the user what is going on
                        clust = clt                        )
ignore = lapply(X = rast_plot,
        FUN = knitr::include_graphics
       )
```
## Plot Averages
...
av_plot = FT_plot_average(path_file = path_av,
                           ...
                           condition1_length = cond_leng, #minutes
                           av_window = av_window,#number of seconds to smooth over for averaging
                           med_window = phase_window,#number of seconds to smooth over for averaging
                           save_type = "png",# "pdf",# 
